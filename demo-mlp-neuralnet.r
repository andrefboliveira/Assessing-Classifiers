...
df <- read.csv2("/Users/taniamaldonado/Desktop/Bioinformática/1º Semestre/Aprendizagem Automática/Assessing-Classifiers/project-default-credit-card-clients.csv", sep = ";")
index <- 1:nrow(df)
...
nn <- neuralnet(paste("Y ~", paste(names[!names %in% c("Y", "X")], collapse=" + ")),
...
# perceptron <- function(D){
#   netwrk <- neuralnet(x3~x1+x2,D,hidden = 2, threshold = 0.01)
#   return(netwrk)
# }  
# 
# trainingdata <- read.csv("/Users/taniamaldonado/Desktop/Bioinformática/1º Semestre/Aprendizagem Automática/Assessing-Classifiers/project-default-credit-card-clients.csv", sep = ";")
# colnames(trainingdata) <- c("x1", "x2", "x3")
# 
# test <- perceptron(trainingdata)
# print(test)
# plot(test) 
